...
def place_order(token: str, symbol: str, exchange: str, side: str, qty: int, price: float, mock: bool = True) -> dict:
    if mock:
        fake_id = f"{int(time.time())}{random.randint(100000, 999999)}"
        return {"status": "success", "data": {"order_id": fake_id}}
    r = requests.post(
        f"{KITE_BASE_URL}/orders/regular", headers=kite_headers(token),
        data={"tradingsymbol": symbol, "exchange": exchange, "transaction_type": side, "order_type": "LIMIT",
              "quantity": qty, "price": price, "product": "CNC", "validity": "DAY"}, timeout=10)
    r.raise_for_status()
    return r.json()

def cancel_order(token: str, order_id: str, mock: bool = False) -> dict:
    if mock: return {"status": "success", "data": {"order_id": order_id}}
    r = requests.delete(f"{KITE_BASE_URL}/orders/regular/{order_id}", headers=kite_headers(token), timeout=10)
    r.raise_for_status()
    return r.json()
    ...
    if mock:
        ...
        for o in (live_orders or []):
            if o.get("state") == "PLACED" and o.get("order_id"):
                filled.append({"order_id": o["order_id"], "tradingsymbol": o["Symbol"], "transaction_type": o["Side"],
                               "quantity": o["Qty"], "average_price": o["Price"],
                               "fill_timestamp": datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")})
        return filled
    r = requests.get(f"{KITE_BASE_URL}/trades", headers=kite_headers(token), timeout=10)
    r.raise_for_status()
    resp = r.json()
    if resp.get("status") != "success": raise ValueError(resp.get("message", "API error"))
    return resp.get("data", [])
